...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateForm() {
        ...
        const fields = [
            { input: fullNameInput, validator: validators.fullName, message: errorMessages.fullName },
            { input: emailInput, validator: validators.email, message: errorMessages.email },
            { input: whatsappInput, validator: validators.whatsapp, message: errorMessages.whatsapp }
        ];

        let isFormValid = true;

        fields.forEach(field => {
            const errorElement = field.input.closest('.form-group').querySelector('.error-message');
    ...
    function showAlreadyRegisteredMessage() {
        ...
        btnEntrarApp.style.cssText = `
            background: linear-gradient(135deg, #ffae00, #ffb84d);
            color: white;
            border: none;
            padding: 12px 24px;
            border-radius: 8px;
        ...
        btnEntrarApp.addEventListener('mouseenter', function() {
            ...
            this.style.boxShadow = '0 8px 25px rgba(255, 174, 0, 0.3)';
        });
